...
///</summary>
var socketconnect;
var flagSocket = false;
// $(document).ready(function () {
//     // socketconnect = io.connect('ws://123.57.255.51:3000');
//     ContentSocket();
// });
//连接soclet
function ContentSocket(SocketUrl) {
  try{
    var New_SocketUrl = SocketUrl==undefined?'http://123.57.255.51:3000':SocketUrl;
    socketconnect = io.connect(New_SocketUrl);
    socketconnect.on('connecting',function(){//：
    ...
    socketconnect.on('disconnect', function() {
        ...
        console.log("与服务其断开,正在尝试重连...");
    });
    socketconnect.on('connect_failed', function() {
        $(".chat-messages").append("<p class='log' style='color:red'>连接失败</p>");
        console.log("连接失败");
    });
    socketconnect.on('error', function() {
  ...
  console.log(flagSocket);
}
var Messaging = new function () {
    //验证消息通道
    //var msg_sdk = new Yunba({ appkey: '55cc0d389477ebf52469582a' });
    //document.write("<script src=\"http://123.57.255.51:3000/socket.io/socket.io.js\" type=\"text/javascript\" ></script>");
    ...
    this.initMsg = function (flashvarsObj, flag) {
      flashvars = flashvarsObj;
      console.log(JSON.stringify(flashvars));
      if(flagSocket){
        ...
        //ContentSocket();
        //Messaging.initMsg(flashvarsObj);
      }
    ...
    this.setAlias = function () {
        ...
        socketconnect.emit('login', { userid: flashvars.id, username: flashvars.name, userRoom: flashvars.rid, usertype: flashvars.type, alias: alias });
        // socketconnect.emit('login', { userid: userobj.id, username: userobj.name, userRoom: talkId, usertype: userobj.type, alias: alias });
        ...
        //监听新用户登录
        socketconnect.on('login', function (o) {
    ...
    this.removeOnlineUserElement = function () {
        //监听用户退出
        socketconnect.on('logout', function (o) {
            var user = username.split("|");
    ...
    this.getOnlineUsers = function () {
        socketconnect.emit('getOnlineUsers', { userRoom: flashvars.rid });
        socketconnect.on('getOnlineUsers', function (data) {
    ...
    this.addMessageElement = function (data, isLog) {
        ...
        if (data.hasOwnProperty("dataContent")) {
            ...
            if (data.dataContent.indexOf("cmd:") >= 0) {
                ...
                if(data.userimg == undefined || data.userimg == "undefined"){
                  data.userimg = "../../res/img/avatar.png";
                }
    ...
    this.publish = function (topic, message) {
        socketconnect.emit('message', { 'userRoom': topic, 'msg': message });
    }

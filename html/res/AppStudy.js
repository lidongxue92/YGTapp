...
var showCourse=true;
//打开知识分类菜单
function openCourseByKnowId(knowId){
    //
    publicCourseParams.knowledgeids = knowId;
    getKnowledgeList(knowId,0);
}
//文件的辅助方法
//打开试卷
...
function openTi(obj){
   ...
   bPlayer.stop();
    ...
    //var arrangeId=window.location.hash.substr(1);
    ...
    var gotoUrl =domain+"/member/index.html#/home/stuquelist/"+arrangeId+"/"+obj.knowledge_Id+"/"+obj.know_select_que_type+"/"+obj.know_select_que_num+"/0/"+obj.knowledge_Name+"/"+arrangeId+"/"+obj.knowledge_Id+"/"+obj.knowledge_Name+"/0/"+sysoUserInfo.user_ID+"/0/phone?a=" + sysoUserInfo.user_ID;
    gotoUrl += "&token="+ strToJson(GetlocalStorage("userinfo_token"));
...
/*********************问答开始*******************************/
var pageIndex=1;//问题的分页
var pageSize=10;//每页显示条数
var pageReplayIndex=1;//回复的分页
var loginUserJson=null;
function getkewenda(){
  $("#user_comment_block ul").html("");
  //获取课程问答列表
  var loginUserJson =getUserInfo();
  var findparams={
		orgid:loginUserJson.organization_ID,
		columnid:csm.key,
		userid:loginUserJson.user_ID,
		powerLV:loginUserJson.powerLV,
		type:"1",
		xtype:1,//问答类型，1课程,
		pageIndex:pageIndex,
		pageSize:pageSize,
  ...
  getAjax(javaserver+"/interlocution/findQuestionList",findparams,function(response){

    var data = JSON.parse(response);
		if(data.errorcode==0){
			HandleHTML(data.datas);//处理页面
		}
		//控制分页加载
		if(data.pageCount==0||data.pageCount<=pageIndex){
...
function HandleHTML(list){
  var loginUserJson =getUserInfo();
	...
	for(var i=0;i<list.length;i++){
		...
		block+="			<li onclick=\"openInterlocution('"+list[i].qaid+"')\">"
			+"					<div class='item-content'>"
			+"							<div class='item-media' >"
			+"								<img src='"+list[i].questionerAvatar+"' style='width: 2rem;border-radius:50%'>"
			+"							</div>"
			+"							<div class='item-inner'>"
			+"								<div class='item-title-row' title = '"+list[i].content+"'><div  class=\"item-title\">"
			+								list[i].title
			+"								</div></div>"
			+"								<div class='item-subtitle' style='color:#666;font-size:12px'><div class=\"row\">"
			+"									<div class='col-33'>"+list[i].questionerName+"</div>"
			+"									<div class='col-33'><i class=\"iconfont icon-pinglun\" style='color:#666;font-size:12px'></i>"
			+"										"+list[i].replyNum+"回答</div>"
			+"									<div class='col-33'>"
			+"										<i class=\"iconfont icon-dqr\" style='color:#666;font-size:12px'></i>"+list[i].createDate.split(" ")[0]+"</div>"
		//	+"									<div class='kecheng_play_mian_bottom_xgwd_right_xqhdwt'  onclick='openComment(this,"+JSON.stringify(list[i])+")'>"
  ...
  if(list.length == 0)
  {
    $("#user_comment_block ul").html("<center style='margin:10px auto'>暂时没有问答信息</center>");
  }
	if($("#user_comment_block ul li").length<=0&&block.length>0){
		$("#user_comment_block ul").html(block);
	}else{
		$("#user_comment_block ul").prepend(block);
	}


		// //提问的回车事件
	// $(".MyReplayText").bind('keyup',function(event){
		// if(event.keyCode == "13") {
			// var item=JSON.parse($(this).attr("data"));
			// replayQuestion(this,item);
		// }
	// });
}
//跳转问答详情页面
function openInterlocution(id) {
    //alert($("body").height()-$("#kecheng_play_mian_top_play").height()-40);
  //  $("#qs_infolist").css({"top":$("#kecheng_play_mian_top_play").height()+40+"px","height":$("body").height() -$("#kecheng_play_mian_top_play").height()-40+'px'}).show();
  //  $("#qs_infolist").find("iframe").attr("src",api.wgtRootDir+"/html/interlocution/interlocutiondetail.html?qaid="+id);
    //$("#qs_infolist").css("top","12rem");
    api.openWin({
        name: 'xxxcc',
        url: api.wgtRootDir+"/html/interlocution/interlocutiondetail.html?qaid="+id
    });
}
function wendahide(){
  $("#qs_infolist").hide();
...
//我要提问
function myAddQuestion(){
	var myText=$("#myCommentText").val();
	myText = myText.replace(/(^\s+)|(\s+$)/g, "");
	if(myText.length <= 0){
		alert("请输入有效内容！");
		return ;
	}
	var myTitle=myText.substring(0, 20);
	if(myText.length<=0){
		return;
  ...
  var loginUserJson =getUserInfo();
	var item = {
                     title: myTitle, // 标题
                     content: myText, // 内容
                     questionerId: loginUserJson.user_ID,
                     questionerName: loginUserJson.user_Name,
                     orgId: loginUserJson.organization_ID,
                     orgName: loginUserJson.organization_Name,
                     questionerAvatar: loginUserJson.user_Img,
                     state: 2, //审核状态
                     isAnonymous: false, //是否匿名
                     xtype: 1, //4.专栏
                     xid: csm.key,
                     xname: csm.courseName
                 };
	getAjax(javaserver+"/interlocution/saveEditQA",{data:JSON.stringify(item)},function(response){
    var data =  JSON.parse(response);
		if(data.errorcode==0 ||data.errorcode=="0"){
			var list=[];

			list.push(data.data);
			HandleHTML(list);//处理页面
			//showAlert("info", "alert", "提示", "提问成功，需要管理员审核后可见，请耐心等待", 200);
			$("#myCommentText").val("");
		}else{
			console.log("新增失败"+JSON.stringify(response));
		}
...
function getkebiji(){
  $("#bijilist").html("");
  var loginUserJson = getUserInfo();
  //alert(csm.key+'X'+csm.playpid)
  getAjax(javaserver+"/courseNote/findCourseNote",{
  				courseId:csm.key, //课程ID
  				orgId:loginUserJson.organization_ID,//企业id
  				sectionId:'0',//当前的小节id
  				userId:loginUserJson.user_ID  //用户id
  			},function(data) {

          data = JSON.parse(data);
  				//提交后处理
  				if(data.errorcode==0){
  					bj_appendHtml(data.datas,1);
  				}else{
  					$("#bijilist").html("");
  					console.log("find list error!!!");
            alert("查询错误"+data.errorcode+'|'+data.errormsg);
  				}
  		});
}
function bijisubmit(){//笔记提交
  var loginUserJson = getUserInfo();
  var text = $("#bijitext").val();
			...
			var data={};
			var bijisimi = $('#bijisimi').is(':checked'); //是否为私有
			//编辑的对象
			var edit=GetlocalStorage("bjobj");
			var item=null;
			if(edit !=undefined && edit.length>0){
				item=eval('('+edit+')');
			}
			if(item!=null&&item.noteId!=undefined){
				item.noteContent=$("#bijitext").val();
				item.isOpen=(bijisimi?1:0);
				data=item;
			}else{
        var PlayCourse = csm; //获得课程对象

        var sectionDataJson={CSNAME:""};
        for(var i=0;i<csm.list.length;i++)
        {
          if(csm.list[i].pid == csm.playpid)
          {
            //alert(csm.list[i]);
            sectionDataJson = csm.list[i];
            break;
          }
        }
        //alert(JSON.stringify(sectionDataJson));
				//当前播放的小节数据
			//	var sectionDataJson = $("#" + csm.playpid)
			//						.attr("data");
      //            alert(sectionDataJson);
			//	sectionDataJson = JSON.parse(sectionDataJson.replace(
			//						new RegExp("'", "gm"), "\""));
      //alert(JSON.stringify(csm));
				data={
					noteContent:$("#bijitext").val(),
					courseId:csm.key,
					courseName:$("#cname").html(),
					sectionId:'0',
					sectionName:sectionDataJson.CSNAME,
					studyTime:0,
					arrangeId:QueryString("arrangeId"),
					screenshot:'',
					userId:loginUserJson.user_ID,
					userName:loginUserJson.user_Name,
					orgId:loginUserJson.organization_ID,
					orgName:loginUserJson.organization_Name,
					userImg:loginUserJson.user_Img,
					isOpen:(bijisimi?1:0),
					state:0
				}
			}

			//courseMemoryObj;当前小节  playTime播放到哪了,pid 小节ID
			getAjax(javaserver+"/courseNote/addCourseNote",{data:JSON.stringify(data)},function(data) {
        //  console.log(data);
          data = JSON.parse(data);
				//提交后处理
				if(data.errorcode==0){
            ...
            //alert(JSON.stringify(data));
						if(item&&item.noteId){
							bj_gelist(csm.playpid);
						}else{
							datas.push(data.data);
							bj_appendHtml(datas,2);
						}
					//关闭页面
					$("#bijitext").val("");
          $.toast("保存成功");
					SetlocalStorage("bjobj","");
				}else{
					alert("add error!!!"+JSON.stringify(data));
				}
			});
}
//1 替换  2,拼接
bj_appendHtml=function(data,type){
  var loginUserJson = getUserInfo();
		var strhtml = "";
		//for遍历下列数据
				for(var i=0;i<data.length;i++){
					...
					strhtml += ("										<img src="+data[i].userImg+" style='width: 2rem;border-radius:50%' />");
					strhtml += ("									</div>");
					strhtml += ("									<div class=\"item-inner\" >");
  					strhtml += ("										<div class=\"item-title-row\">");
  					if(data[i].screenshot != null && data[i].screenshot != ""){
              //  alert(data[i].screenshot);
  						strhtml += ("											<img src='"+data[i].screenshot+"' class=\"bijiimg\" style='max-width:100%' />");
  					}


  					strhtml += ("										<div class=\"item-title\">"+data[i].noteContent+"</div>");
  					strhtml += ("										</div>");
					strhtml += ("										<div class=\"item-subtitle\">");
				//	strhtml += ("											<span>"+data[i].userName+"</span>");
					strhtml += ("											");
					strhtml += ("											<span style='color:#666;font-size:12px'>");
					strhtml += ("												<i class=\"iconfont icon-dqr\" style='color:#666;font-size:12px'></i>"+data[i].createDate+"</span>");
				//	strhtml += ("											<span>");
				//	strhtml += ("												播放时间："+arrive_timer_format(data[i].studyTime)+"</span>");
					strhtml += ("											<span onclick=\"bijidelte(this,\'"+data[i].noteId+"\')\">");
					strhtml += ("");
					if(data[i].userId==loginUserJson.user_ID){
				//	strhtml += ("												<span onclick='bijiobj.bijiedit(this,"+JSON.stringify(data[i])+")'><svg class=\"icon\" aria-hidden=\"true\">");
				//	strhtml += ("													<use xmlns:xlink=\"http://www.w3.org/1999/xlink\" xlink:href=\"#icon-bianji\"></use>");
				//	strhtml += ("												</svg> 修改</span>");
					strhtml += ("												 <i class=\"iconfont icon-shanchu1\"></i> 删除</span>");
					}
					strhtml += ("												</div>");
					strhtml += ("										</div>");
				...
				if(type==1){
					$("#bijilist").html(strhtml);
				}else{
					$("#bijilist").prepend(strhtml);
				}

	}
  function bijidelte (obj,id){
		getAjax(javaserver+"/courseNote/delCourseNotes",{noteIds:id},function(data) {
      data = JSON.parse(data);
				//提交后处理
				if(data.errorcode==0){
            $.toast("删除成功");
...
$(document).on("pageInit", "#renwu_detail", function(e, id, $page) {
  ...
  $(".bofanglogo").attr("src", javafile+"/resources/logo/"+getUserInfo().organization_ID+"_2.png?r="+Math.ceil());
    initStartTime();
    ...
    if(studyArrangeId!=null&&studyArrangeId!=undefined){
   ...
   // $.showPreloader();//显示Loading
    ...
    getAjax(javaserver+"/course/findCourseProperties",{
            ...
            arrangeId:window.location.hash.substr(1),
    ...
    if(coursename.length>12)
    {
        ...
        $("#cname").html(coursename.substring(0,12)+'..');
    ...
    for (var i =0; i< detailedJSON.length ; i++) {
        ...
        if(kelist.length >0){
            for (var j = 0; j<kelist.length; j++) {
               ...
               var xiazaihtml = "";
              // alert(csobj.CSURL)
               if(csobj.CSURL != undefined && csobj.CSURL.indexOf('.htm')<0 && csobj.CSTYPE ==1){
                  xiazaihtml = "<div class='button ' style=\"margin-left: -0.5rem;margin-right:5px\"><i class=\"iconfont icon-xiazai\" style=\"font-size:18px;\" data='"+JSON.stringify(csobj)+"' onclick=\"huancunxiazai(this,'course')\"> </i></div>";
               }
               if(csobj.CSURL != undefined && csobj.CSURL.indexOf('.htm')<0 && csobj.CSTYPE ==2){
                  xiazaihtml = "<div class='button ' style=\"margin-left: -0.5rem;margin-right:5px\"><i class=\"iconfont icon-xiazai\" style=\"font-size:18px;\" data='"+JSON.stringify(csobj)+"' onclick=\"huancunxiazai(this,'course')\"> </i></div>";
               }
               if(csobj.CSNAME.length>10)
               {
                 csobj.CSNAME = csobj.CSNAME.replace(/<\/?[^>]*>/g,'').substring(0,10)+'..';
               }
            ...
            function(data) {
               ...
               // 读取本地缓存，替换原来的
                ...
                if(localmemary!=null&&localmemary!=""&&localmemary!=undefined&&localmemary.list!=undefined&&localmemary.list.length>0){
                      if(studyJsonDetailsJson==null||studyJsonDetailsJson==undefined||studyJsonDetailsJson==""||new Date(studyJsonDetailsJson.datetime)<new Date(localmemary.datetime)){
               ...
               if(!isPaper){
  				        ...
  				        if($("#kecheng_" + csm.playpid)){
  					        $("#kecheng_" + csm.playpid).find('.item-inner').click();
  				        }else{
  					      //  $("#kechengmingxi .item-content").eq(0).click();
                      $("#kechengmingxi .item-content").eq(0).find('.item-inner').click();
  				        }
               }
                } else {

                  // 发现记忆的课程小节不存在的时候，跳到第一小节
                    // 找不到播放记忆，默认播放第一个
  ...
  $(".qingxidu").hide();
   ...
   // console.log(xiaojie);
          ...
          if(!locvideo) //不需要就穿false
          {
            ...
            if(jsonlist.errorcode == "0" && jsonlist.datas.length >0){
                    ...
                    $.each(jsonlist.datas,function(i,item)
                    {
                        ...
                        // }
                        //******************过滤本地视频****************************//
                        if(item.filepreview.indexOf(javafile) >= 0){  //过滤掉服务器上的本地文件路径，可能是转码失败的视频
                            if(!locvideo) //不需要就穿false
                            {
                              return true;
                            }
                    ...
                    //xxxxxxxxxxxxxx switchVideo(okayUrl,qxd);
            ...
            if(csm.list.length > 0){
                for(var i = 0; i < csm.list.length; i++){
                    if(csm.list[i].pid == xiaojie.CSID){ //说明记忆缓存中有数据
                        ...
                        BaiDuPlayer.BDtimego(csm.list[i].learnTime);
                        break;
                    }
                }
            }
    ...
    case 3:
        ...
        $("#playText").hide();
        $("#kechengContent").css("top","12rem");
        $("#kecheng_play_mian_top_play").html("<div class='shanguang'><img src='"+javafile+"/resources/logo/"+getUserInfo().organization_ID+"_2.png' alt='加载中...'><div class='baiguang'></div></div>");
    ...
    case 6:
        ...
        console.log(livetimeout);
            ...
            var huifangUrl = xiaojie.liveObj.record_htm+"?f=app&uname="+encodeURI(encodeURI(getUserInfo().user_Name))+"&uid="+getUserInfo().user_ID;
          ...
          //$.confirm("本章节为直播课程，是否观看直播？", function () {
            var type = xiaojie.livetype;
            if(type == "1"){
              //ppt+语音模式直播
              api.openWin({name:'liveroom',url: api.wgtRootDir+'/html/live.html',pageParam: {
                roomid: xiaojie.liveObj.liveid,
                socketurl:xiaojie.liveObj.socketurl
              }});
            }
            else if(type == "2") {
              //打开纯视频模式直播
              SetlocalStorage("LiveBroadcast_Info", JSON.stringify(JSON.stringify(xiaojie)));  //将信息存入缓存，以便直播界面直接获取使用
              api.openWin({
                  name: 'live',
                  animation:{
                     type:"none"
                  },
                  url: api.wgtRootDir+'/html/peixun/live.html?f=l'
              });
            }
            else if(type == "3") { //小鱼互联
              api.openWin({
                  ...
                  pageParam: {
                      title:xiaojie.CSNAME,
                      url: xiaojie.liveObj.stu_htm
                  }
              });
            }
            else {
            ...
            //$.router.loadPage("../../html/peixun/live.html?arrangeId="+window.location.hash.substr(1));
          //});
    		}
        break;
    case '8':
    ...
    if(!QueryString("courseId")){
      ...
      //$.confirm("本章节为在线考试，是否确认打开试卷？", function () {
        ...
        if (courseDetailedJson.CRANDIM == 0) {
            ...
            sjurl = javafile+"/resources/exam/" + courseDetailedJson.CSPID + "," + courseDetailedJson.CSPCOUNT + "/" + num + ".html?cid=" + csm.key + "&playid="+playid+"&userid="+userid+"&arrangeId=" + QueryString("arrangeId")+"&token="+strToJson(GetlocalStorage("userinfo_token"));
        } else {
            sjurl = javafile+"/resources/exam/" + courseDetailedJson.CSPID
                    + "/" + courseDetailedJson.CSPID + ".html?cid="
                    + csm.key + "&playid="+playid+"&userid="+userid+"&arrangeId=" + window.location.hash.substr(1)+"&token="+strToJson(GetlocalStorage("userinfo_token"));
        }
        api.openWin({
            name: 'kaishi',
            url: "../html/iframe.html",
            ...
            pageParam: {
                title:strToJson(courseDetailedJson.chapterJson)[0].paperName,
                url: sjurl
            }
        });
...
function jishiqi(jilu){
    ...
    for (var i = csm.list.length - 1; i >= 0; i--) {
        if(csm.list[i].pid == jilu.pid)//找到这个记录
        {
            ...
            //找到记录更新本地缓存
            //找到记录更新本地缓存
            csm.datetime = Date.parse(new Date());;
       ...
       getAjax(javaserver+
        ...
        if(!showCourse&&(arrId==undefined||arrId==null||arrId==""||arrId=="undefined"||arrId==1||arrId=="1")){
             getAjax(javaserver+"/coursetime/addLogs",
                {userid:getUserInfo().user_ID,content:"任务id异常，APP提交参数："+JSON.stringify(canshu)+"，页面地址："+window.location.href},
                function(data){
                    if(data.errorcode!="0"){
...
var mainplayer = function(Cantent, Height, mima) {
  ...
  window.AddHtml = function(Cantent, Height, mima) {
      ...
      // / </summary>
      $("#" + Cantent).html("");
      //$("#" + Cantent).append(  "<iframe class=\"embed-responsive-item\" style=\"height:16rem\" src='" + mima  + "'></iframe>");
      // 设置外框架
      ...
      api.setFrameClient({
          ...
          switch (ret.state) {
              ...
              case 1:
                  break;
              case 2:
                  var classnames = "try{	document.getElementById('firstPanel').className='';}catch(e){}	try{document.getElementById('allPanel').className='';}catch(e){}	"
  ...
  window.AddSanFang = function(Cantent, Height, mima) {
      ...
      // / </summary>
       ...
       + mima + "\" target=\"_blank\" role=\"button\"><i class=\"glyphicon glyphicon-download\"></i> 不支持的格式</a></center></div>");
       try{
         bPlayer.stop();
       }catch(e){}
  }
  window.AddOffice = function(Cantent, Height, mima) {
     // 设置外框架
      ...
      // / 增加文档播放组件
      // / </summary>
       $("#" + Cantent).html("<iframe class=\"embed-responsive-item\"  src='" +  "../../script/pdf2/web/viewer.html?file=" +
  ...
  if (mima.indexOf('.htm')>0 || houzhui == "htm" || houzhui == "html" || houzhui == "shtml") {
      // 静态类内容
...
//封装百度播放器
var bPlayer = {};
var quanping = false;//全屏状态
var BaiDuPlayer = new function(){
try{
  //播放
  this.play = function (Cantent, Height, mima){
    $("#" + Cantent).html("<div id='a1'></div>");
    //  console.log(mima.replace('\\', '/'));
    //var player;
    Tstate = 0;
    bPlayer = cyberplayer("a1").setup({
       width: '100%', // 宽度，也可以支持百分比(不过父元素宽度要有)
       height: playerHeight, // 高度，也可以支持百分比
       file:mima.replace('\\', '/'), //'http://gcqq450f71eywn6bv7u.exp.bcevod.com/mda-hbqagik5sfq1jsai/mda-hbqagik5sfq1jsai.mp4', // 播放地址
  //     image: "http://gcqq450f71eywn6bv7u.exp.bcevod.com/mda-hbqagik5sfq1jsai/mda-hbqagik5sfq1jsai.jpg", // 预览图
       ...
       ak: "7200a73f5c7042b9a1cddd897de56bb3" // 公有云平台注册即可获得accessKey
   });
   try {
     bPlayer.play();
   } catch (e) {
	 ...
	 bPlayer.onReady(function(){
  	 bPlayer.onPlay(function () { Ckplay(); });   //设置播放器播放事件监听函数
  	 bPlayer.onPause(function () { Ckpause(); }); //设置播放器暂停事件监听函数
  	 bPlayer.onPlay(function () {  Tstate = 1; timego(); }); //设置播放器播放事件监听函数
     bPlayer.on('error', function (err) {
        alert(err.message);
     });
     bPlayer.on('fullscreen', function (rs) {
        if(rs.fullscreen)
        {
  	 ...
  	 bPlayer.onComplete(function () { VideoPlayEndedHandler(); }); //设置播放器完成事件监听函数
   });
    /**
    try{
      bPlayer.close();
    ...
    try{
      ...
      bPlayer = winapi1.require('bPlayer');  //实例化百度播放器
    }
    ...
    console.log(mima);
		...
		bPlayer.open({
              rect:{
                ...
                {
                  console.log('开始了:'+JSON.stringify(ret));
    ...
    //           h: 0,
    //       },
    //       path: mima.replace('\\', '/'),//mima.replace('\\', '/'),
    //       autoPlay: true
    //   }, function(ret, err) {
    //       if (ret) {
...
function openKe(stringjson,renwuid) {
    ...
    SetlocalStorage("studyArrangeId",renwuid);
    ...
    if(lastH != "home"){
      ...
      //  alert(window.location.href);
        $.router.loadPage(api.wgtRootDir+"/html/peixun/detail.html?arrangeId="+renwuid);
    //  $.router.loadPage("peixun/detail.html?arrangeId="+renwuid);
    }
}
//从收藏  打开课程播放界面
//需要获取  课程阶段 信息
...
function huancunxiazai(obj, flag) {
  ...
  if (data != null) {
    ...
    if(flag == "course"){
      ...
      {
        var wenjianming = data.filepreview.split('/');
        var name = wenjianming[wenjianming.length-1];
        //alert(data.filepreview);
        savefile_jsonObj = {
          "url": data.filepreview,
          "savePath": api.cacheDir+'/down/'+name,
          "iconPath":'',
          "cache": false,
          "allowResume": true,
          "title": name,
        ...
        savefile_url = api.cacheDir+"/"+ flag + "/" + data.upId + "/" + data.fileName;//fsDir,api.cacheDir
        savefile_jsonObj = {
          "url": data.filepreview,
          "savePath": savefile_url,
  ...
  //  alert(JSON.stringify(savefile_jsonObj));
...
function renwu_detail_black(){
   ...
   try{bPlayer.stop();}catch(e){}
   //
   try{
    // window.history.go(-1);
     $.router.back("../homenew.html");
   }
   catch(e){
